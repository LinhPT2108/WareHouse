...
public class ImportBillDAO extends ArtDAO<ImportBill, String> {
    ...
    public void insert(ImportBill e) {
        String query = " INSERT INTO dbo.DonNhapHang ( MaDonNhap ,MaNV ,MaKhoHang ,TongTienNhap ,TrangThaiThanhToan ,SoTienDaThanhToan ,GhiChu ,NgayTao ,NguoiTao)"
                + "values (?,?,1,?,?,?,?, getDate(),?)";
        XJDBCHelper.executeUpdate(query, e.getImportBillID(), e.getEmployeeID(), e.getTotalAmount(), e.getPayMentStatusID(), e.getTotalPay(), e.getNote(), e.getCreatedBy());
    }
    ...
    public ImportBill selectById(String key) {
        List<ImportBill> list = this.selectBySql("SELECT *FROM dbo.DonNhapHang where MaDonNhap = ?", key);
        if (list.isEmpty()) {
            return null;
        }
        return list.get(0);
    ...
    protected List<ImportBill> selectBySql(String sql, Object... args) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                iBill.setPayMentStatusID(rs.getString("TrangThaiThanhToan"));
    ...
    public List<ImportBill> selectAllASC() {
        String query = """
                       SELECT * FROM dbo.DonNhapHang Where TrangThaiXoa = 0 ORDER BY MaDonNhap ASC
                       """;
        return selectBySql(query);
    }
    
    public List<ImportBill> selectPayedASC() {
        String query = """
                       SELECT * FROM dbo.DonNhapHang Where TrangThaiXoa = 0 AND TrangThaiThanhToan=1 ORDER BY MaDonNhap ASC
                       """;
        return selectBySql(query);
    }
    public List<ImportBill> selectPayingASC() {
        String query = """
                       SELECT * FROM dbo.DonNhapHang Where TrangThaiXoa = 0 AND TrangThaiThanhToan=2 ORDER BY MaDonNhap ASC
                       """;
        return selectBySql(query);
    }
    public List<ImportBill> selectNoPayASC() {
        String query = """
                       SELECT * FROM dbo.DonNhapHang Where TrangThaiXoa = 0 AND TrangThaiThanhToan=3 ORDER BY MaDonNhap ASC
                       """;
        return selectBySql(query);
    }
    
    public List<ImportBill> selectAllDESC() {
        String query = """
                       SELECT * FROM dbo.DonNhapHang Where TrangThaiXoa = 0 ORDER BY MaDonNhap DESC
                       """;
        return selectBySql(query);
    }
    
    public List<ImportBill> selectPayedDESC() {
        String query = """
                       SELECT * FROM dbo.DonNhapHang Where TrangThaiXoa = 0 AND TrangThaiThanhToan=1 ORDER BY MaDonNhap DESC
                       """;
        return selectBySql(query);
    }
    public List<ImportBill> selectPayingDESC() {
        String query = """
                       SELECT * FROM dbo.DonNhapHang Where TrangThaiXoa = 0 AND TrangThaiThanhToan=2 ORDER BY MaDonNhap DESC
                       """;
        return selectBySql(query);
    }
    public List<ImportBill> selectNoPayDESC() {
        String query = """
                       SELECT * FROM dbo.DonNhapHang Where TrangThaiXoa = 0 AND TrangThaiThanhToan=3 ORDER BY MaDonNhap DESC
                       """;
        return selectBySql(query);
    }
}
