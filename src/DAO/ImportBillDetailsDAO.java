...
public class ImportBillDetailsDAO extends ArtDAO<ImportBilldetails, String> {
    ...
    public void insert(ImportBilldetails e) {
        ...
        XJDBCHelper.executeUpdate(query, e.getImportBillDetailsID(), e.getImportBillID(), e.getProductID(),
                ...
                e.getIntoMoney(), XDateHelper.toString(XDateHelper.toDate(e.getManufacturingDate(), "yyyyMMdd")),
                XDateHelper.toString(XDateHelper.toDate(e.getExpiryDate(), "yyyyMMdd")), e.getNotes());
    }
    ...
    public void update(ImportBilldetails e) {
        XJDBCHelper.executeUpdate("""
                                    ...
                                    SoLuong = ?, ThanhTien = ?, NgaySanXuat = CONVERT(DATE, ?),HanSuDung = CONVERT(DATE, ?), ChiTiet = ? 
                ...
                e.getIntoMoney(), XDateHelper.toString(XDateHelper.toDate(e.getManufacturingDate(), "yyyyMMdd")),
                XDateHelper.toString(XDateHelper.toDate(e.getExpiryDate(), "yyyyMMdd")), e.getNotes(),
                e.getImportBillDetailsID(), e.getImportBillID());
    }
    ...
    public List<ImportBilldetails> selectByHD(String idImport) {
        String query = "SELECT * FROM dbo.ChiTietNhap WHERE MaDonNhap =?";
        return selectBySql(query, idImport);
    }
